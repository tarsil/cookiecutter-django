...
from accounts.types import ProfileType
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        for field in self.UserModel.REQUIRED_FIELDS:
            parser.add_argument(
                "--%s" % field, dest=field, default=None, help="Specifies the %s for the superuser." % field
            )
        ...
        if not options["interactive"]:
            ...
            try:
                ...
                while username is None:
                    ...
                    input_msg = "%s%s: " % (
                        input_msg,
                        " (%s.%s)" % (username_rel.model._meta.object_name, username_rel.field_name)
                        if username_rel
                        else "",
                    )
    ...
    def generate_profile_from_user(self, **user_data):
        ...
        HubUser.objects.create(
            user=user, slug=slugify(user.username), uuid=str(uuid.uuid4()), profile_type=ProfileType.ADMIN
        )
